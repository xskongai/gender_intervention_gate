...
# Project label mapping: EDIT -> POSITIVE, KEEP -> NEGATIVE.
_DECISION_TO_LABEL = {"EDIT": "POSITIVE", "KEEP": "NEGATIVE"}


def deterministic_label(text: str) -> dict[str, str] | None:
    """Return a project-native POSITIVE/NEGATIVE rule result, or ``None``.

    The deterministic rules only handle sufficient-condition cases. ``None``
    means that the existing frozen LLM Gate must decide the item.
    """
    result = deterministic_decision(text)
    if result is None:
        return None
    return {
        "label": _DECISION_TO_LABEL[result["decision"]],
